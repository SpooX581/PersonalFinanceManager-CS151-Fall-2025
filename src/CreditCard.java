...
public class CreditCard extends AbstractCreditCard {
    ...
    public void dueDate() {
        ...
        dueDate = closingDate + gracePeriod; // due date is after grace period is over, which is after billing cycle is
                                             // over (add billing cycle & grace period together)
        if (dueDate > 28) { // 28 days in a month, so if greater than 28 days, due date will be at 28 (max
                            // day)
            dueDate = 28;
        }
    ...
    public void minimumPayment() {
        ...
        System.out.println("Your minimum payment due for this month to avoid late fee is: $"
                + String.format("%.2f", paymentPercent)); // smallest payment due each month in order to have no late
                                                          // fee
    }
